...
int julia(double cr, double ci, double zr, double zi, int max_iterations) {
    int iterations = 0;
...
cv::Vec3b color_scheme(int iterations) {
    if (iterations >= 255) {
        return cv::Vec3b(0, 0, 0);
    }
    else {
        uchar r = static_cast<uchar>((iterations % 7 + 1) / 8.0 * 255);
        uchar g = static_cast<uchar>((iterations % 5 + 1) / 6.0 * 255);
        uchar b = static_cast<uchar>((iterations % 11 + 1) / 4.0 * 255);
...
int main(int argc, char** argv) {
    ...
    while (redraw) {
        ...
        for (int j = start_row; j < end_row; j++) {
            for (int i = 0; i < width; i++) {
                ...
                int iterations = julia(cr, ci, x, y, max_iterations);

                sub_image.at<cv::Vec3b>(j - start_row, i) = color_scheme(iterations);
            }
        }
        ...
        if (rank == 0) {
            cv::namedWindow("Julia Fractal", cv::WINDOW_NORMAL);
            imshow("Julia Fractal", final_image);
    ...
    if (rank == 0) {
        if (!cv::imwrite("D:/fractal.png", final_image)) {
